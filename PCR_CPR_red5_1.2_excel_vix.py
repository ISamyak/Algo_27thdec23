...
folder_path = './Nirmal_Fincap/NIFTY_ADJ_OPT_2021/4/29APR21'
nifty_path = './'
...
def setting_strike_price(setting_sp_closeTime,selected_strike_price,m,n):
	...
	if india_vix>21.5:
		...
		if a==['']:
			...
			selected_check.append(for_CE_STRIKE+300)
			selected_check.append(for_CE_STRIKE+400)
			...
			selected_check.append(for_PE_STRIKE-300)
			selected_check.append(for_PE_STRIKE-400)
			# selected_check.append(for_CE_STRIKE-150)
...
def setting_strike_price1(setting_sp_closeTime,selected_strike_price,m,n):
	# setting_sp_startTime = output_date+ " 091500"
	# a=getting_ohlc_nifty(setting_sp_startTime,setting_sp_closeTime)
	a=getting_nifty_data_from_excel(date,m,n)
	if a==['']:
...
def set_data_for_max_oi(OI_Data,selected_check,india_vix):
	if india_vix <= 21.5:
		...
		A=np.array(OI_Data)
		row_info0=np.where(A==selected_check[0])[0].tolist()
		row_info1=np.where(A==selected_check[1])[0].tolist()
		row_info2=np.where(A==selected_check[2])[0].tolist()
		row_info3=np.where(A==selected_check[3])[0].tolist()
		row_info4=np.where(A==selected_check[4])[0].tolist()
		row_info5=np.where(A==selected_check[5])[0].tolist()
		row_info6=np.where(A==selected_check[6])[0].tolist()
		row_info7=np.where(A==selected_check[7])[0].tolist()
		row_info8=np.where(A==selected_check[8])[0].tolist()
		row_info9=np.where(A==selected_check[9])[0].tolist()

		oi_info=[OI_Data[row_info0[0]][7],OI_Data[row_info1[0]][7],OI_Data[row_info2[0]][7],OI_Data[row_info3[0]][7],OI_Data[row_info4[0]][7]]
		max_value=max(oi_info)
		max_index=oi_info.index(max_value)

		if max_index==0:
			max_oi_call=OI_Data[row_info0[0]][7]
			last_day_call_oi=OI_Data[row_info0[0]][1]
			indx_ce=row_info0
		elif max_index==1:
			max_oi_call=OI_Data[row_info1[0]][7]
			last_day_call_oi=OI_Data[row_info1[0]][1]
			...
			max_oi_call=OI_Data[row_info4[0]][7]
			last_day_call_oi=OI_Data[row_info4[0]][1]
			indx_ce=row_info4				


		oi_info=[OI_Data[row_info5[0]][8],OI_Data[row_info6[0]][8],OI_Data[row_info7[0]][8],OI_Data[row_info8[0]][8],OI_Data[row_info9[0]][8]]
		max_value=max(oi_info)
		max_index=oi_info.index(max_value)
		...
		if max_index==0:
			...
			max_oi_put=OI_Data[row_info7[0]][8]
			last_day_put_oi=OI_Data[row_info7[0]][2]
			indx_pe=row_info7
		elif max_index==3:
			max_oi_put=OI_Data[row_info8[0]][8]
			last_day_put_oi=OI_Data[row_info8[0]][2]
			indx_pe=row_info8
		elif max_index==4:
			max_oi_put=OI_Data[row_info9[0]][8]
			last_day_put_oi=OI_Data[row_info9[0]][2]
			indx_pe=row_info9
...
def set_data_for_max_change_in_oi1(OI_Data,selected_check):
	A=np.array(OI_Data)
	row_info1=np.where(A==selected_check[0])[0].tolist()
	row_info2=np.where(A==selected_check[1])[0].tolist()
	row_info3=np.where(A==selected_check[2])[0].tolist()
	row_info4=np.where(A==selected_check[6])[0].tolist()
	row_info5=np.where(A==selected_check[7])[0].tolist()
	row_info6=np.where(A==selected_check[8])[0].tolist()
...
def trade_initiate(date,a,side,time):
	...
	if side=='PE.NFO':
		selling_sp = float(a[3]) - (float(a[3])%50) -400
		buying_sp=selling_sp - 100
	else:
		selling_sp= float(a[3])  + (50 - float(a[3])%50) + 400
		buying_sp=selling_sp+100	
...
with open(filepath, 'w', newline='') as file:
	...
	for i in files:
		...
		if float(india_vix) > 21.5:
			print("Take extra sp")
			"""Setting Strike Price For Day"""
			output_closing_time = output_date + " 093000"
			selected_strike_price = setting_strike_price1(output_closing_time,[],0,3)
			...
			if gap=="UP":
				...
				extra_sp_pe=selected_strike_price[5]+100
				print(extra_sp_pe)	
			...
			"""Setting Strike Price For Day"""
			output_closing_time = output_date + " 093000"
			selected_strike_price = setting_strike_price(output_closing_time,[],0,3)
			print(selected_strike_price)
			...
			if gap=="UP":
				...
				extra_sp_pe=selected_strike_price[3]+100
				print(extra_sp_pe)	
		...
		while time_obj.time() < datetime.strptime('14:55:59', '%H:%M:%S').time():
			...
			if float(india_vix)>21.5:
				max_oi_call_chng,last_day_call_oi_chng,indx_ce_chng,max_oi_put_chng,last_day_put_oi_chng,indx_pe_chng= set_data_for_max_change_in_oi1(OI_Data,selected_strike_price)
			else:
			...
			if ( crossover_by_ce and extra_sp_pe==0):
				if time_of_crossover==time_obj:
					...
					if (set_oi_put * 0.95) > OI_Data[indx_pe[0]][8]:
						...
						if count==0:
							...
							a1=getting_nifty_data_from_excel(date,crossover_n,n+1)
							if a1==['']:
								print("Ignore once")
							else:	
				...
				row_info=np.where(B==extra_sp_pe)[0].tolist()
				if time_of_crossover==time_obj:
			...
			if ( crossover_by_pe and extra_sp_ce==0):
				...
				row_info=np.where(B==extra_sp_ce)[0].tolist()
				if time_of_crossover==time_obj:
			...
			if trade and square_off==False:
				if trade_side=='PUT':
					...
					if (reversal_point_initiated) and square_off==False:
						...
						a1=getting_nifty_data_from_excel(date,n-1,n)
						print("Square of position below :" + str((float(a1[3]))-10))
					...
					if (reversal_point_initiated) and square_off==False:
						...
						# a=nifty_data_after_format_change(time_int2,time_obj)
						a1=getting_nifty_data_from_excel(date,n-1,n)
						print("Square off Position above :" + str((float(a1[3]))+10))
						trade_point=float(a1[3])
						reversal_point_initiated_time=time_obj
						square_off=True	
			...
			formatted_num = '{:06d}'.format(time_int)
			output_closing_time=output_date + " "+str(formatted_num)
			selected_check1=setting_strike_price(output_closing_time,selected_strike_price,0,n)
