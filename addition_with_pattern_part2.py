...
folder_path = './Nirmal_Fincap/NIFTY_ADJ_OPT_2022/4/expiry1/A'
...
with open(filepath, 'w', newline='') as file:
	...
	for i in files:
		...
		selected_itm_Sp = select_itm_strike_price(output_closing_time,[],0,46)
		...
		while time_obj.time() < datetime.strptime('14:55:59', '%H:%M:%S').time():
			...
			if count15_C>=3 and range_bound_C15:
			...
			if count15_P>=3 and range_bound_P15:
			...
			if count20_C>=3 and range_bound_C20:
				if moving_avg_function(date,n,'P'):
					print("Trend reverse")
					reverse20_C=True
			...
			if count20_P>=3 and range_bound_P20:
				if moving_avg_function(date,n,'C'):
					print("Trend reverse")
					reverse20_P=True
			...
			if count30_C>=3 and range_bound_C30:
				if moving_avg_function(date,n,'P'):
					print("Trend reverse")
					reverse30_C=True
			...
			if count30_P>=3 and range_bound_P30:
				if moving_avg_function(date,n,'C'):
					print("Trend reverse")
					reverse30_P=True
