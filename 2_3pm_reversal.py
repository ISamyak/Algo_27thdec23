...
folder_path = './Nirmal_Fincap/NIFTY_ADJ_OPT_2022/10/ex-date'
...
def trade_initiate(date,tick,time):
	...
	if time_obj.time() < datetime.strptime('14:45:59', '%H:%M:%S').time():
		print("100")
		if "CE" in tick:
			tick1=int(tick[12:17]) + 100
			tick1=tick[0:12]+str(tick1)+tick[17:]
			tick=tick1
		if "PE" in tick:
			tick1=int(tick[12:17]) - 100
			tick1=tick[0:12]+str(tick1)+tick[17:]
...
with open(filepath, 'w', newline='') as file:
	...
	for i in files:
		...
		while time_obj.time() < datetime.strptime('15:29:59', '%H:%M:%S').time():
			...
			if(Table[1][1] > 0 or Table[2][1]>0) and (Trade=='') and Get_point1==False:
				print("I am here")
				...
				if Table[1][1]>5 and (Table[1][1] - Table[5][1] >= k):
					...
					while Get_point1==False and time_obj1.time() > datetime.strptime('09:16:59', '%H:%M:%S').time():
						...
						if extracted_data1==[]:
							...
							continue
						min_atp_ltp_x=min(min_atp_ltp_x,extracted_data1[0][2])
			...
			if(Table[5][1] > 0 or Table[6][1]>0) and (Trade=='') and Get_point==False:
				print("I am here")	
				...
				if Table[6][1]>5 and (Table[6][1] - Table[2][1] >= k1):
					...
					while Get_point== False and time_obj1.time() > datetime.strptime('09:16:59', '%H:%M:%S').time():
						...
						if extracted_data1==[]:
							time_obj1 -= timedelta(minutes=candle)	
							continue
						min_atp_ltp_y=min(min_atp_ltp_y,extracted_data1[0][2])
