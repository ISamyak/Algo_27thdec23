...
folder_path = './Nirmal_Fincap/NIFTY_ADJ_OPT_2023/3_7/ex-date-fn/C'
...
def setting_oi_change_data_inner(length,filter_based_on_sp,selected_check,OI_Data,time):
	...
	for i in range(int(length/2)):
		...
		if(extracted_data != []):
			...
			if (True if float(filter_based_on_sp[i][15:20]) in selected_check else False):
		...
		if(extracted_data != []):
			...
			if (True if float(filter_based_on_sp[i+5][15:20]) in selected_check else False):
...
def update_oi_data(length,filter_based_on_sp,selected_check,OI_Data,time):
	...
	for i in range(int(length/2)):
		...
		if(extracted_data != []):
			...
			if (True if float(filter_based_on_sp[i+9][15:20]) in selected_check else False):
				print("9th Wrong Data")				    
	return OI_Data
...
def oi_subtraction_flag(OI_Data,index):
	...
	if index%2==0 and (index-2)>0 and index<10:
...
with open(filepath, 'w', newline='') as file:
	...
	for i in files:
		...
		while time_obj.time() < datetime.strptime('15:00:59', '%H:%M:%S').time():
			...
			if time_obj.time() >= datetime.strptime('10:09:59', '%H:%M:%S').time():
				max_index=0
				max_oi=-100000000000000
				...
				if flag_side=='' and time_obj.time() < datetime.strptime('14:00:59', '%H:%M:%S').time():
							...
							if index%2==0 and flag_side=='CE':
								...
								if index- min_oi_index > 2:
									...
									if float(a[4])-35 <= OI_Data_inner[int(index/2)][0] <= float(a[4])+35 :
										...
										if ( OI_Data_inner[int((index-2)/2)][3]>1000000 and (float(a[4])-35 <= OI_Data_inner[int(index/2)][0] <= float(a[4])+35)):
									...
									if float(a[4])-35 <= OI_Data_inner[int(index/2)][0] <= float(a[4])+35 :
										...
										if float(a[4])-35 <= OI_Data_inner[int((index-2)/2)][0] <= float(a[4])+35 :
								...
								if 	min_oi_index - index >2:
									...
									if float(a[4])-35 <= OI_Data_inner[int(index/2)][0] <= float(a[4])+35:
										...
										if ( OI_Data_inner[int((index+2)/2)][4]>1000000 and float(a[4])-35 <= OI_Data_inner[int(index/2)][0] <= float(a[4])+35  ):
