...
folder_path = './Nirmal_Fincap/NIFTY_ADJ_OPT_2022/10/Expiry3'
prefix="NIFTY20OCT22"
m=1
...
def ratio_125_check(r1,r2,indx,indx_chng,s1,s2):
	cumulative=False
	ratio= abs(r1)/abs(r2)
	if ratio>=1.25:
		cumulative=True
	...
	for i in files:
		...
		while time_obj.time() < datetime.strptime('14:55:59', '%H:%M:%S').time():
			...
			if crossover_by_pe:
				...
				Cumulative_for_PE = ratio_125_check(max_oi_put,max_oi_call,indx_ce,indx_ce_chng,max_oi_put_chng,max_oi_call_chng)
				if Cumulative_for_PE:
					print("PCR Ratio")
			if crossover_by_ce:
				max_change_ce=abs(max_oi_call_chng - last_day_call_oi_chng)
				max_change_pe=abs(max_oi_put_chng - last_day_put_oi_chng)
				Cumulative_for_CE = ratio_125_check(max_oi_call,max_oi_put,indx_pe,indx_pe_chng,max_oi_call_chng,max_oi_put_chng)
			...
			"""Reversal"""
			# if trade:
			# 	OI_Data_15min = update_oi_data(length,filter_based_on_sp,selected_strike_price,OI_Data_15min,time_obj.time())
			# 	time_after_15_min=time_obj + timedelta(minutes=15)
			# 	OI_Data_15min = update_oi_data(length,filter_based_on_sp,selected_strike_price,OI_Data_15min,time_after_15_min.time())

			# 	if trade_side=='PUT':
			# 		rows=np.where(B==selected_strike_price[3])[0].tolist()
			# 		rows1=np.where(B==selected_strike_price[4])[0].tolist()
			# 		rows2=np.where(B==selected_strike_price[5])[0].tolist()
			# 		Reduction_Count=0
			# 		Reduction_Count = Reduction_Count+1 if OI_Data_15min[rows[0]][6]-OI_Data_15min[rows[0]][4] > 0 else Reduction_Count
			# 		Reduction_Count = Reduction_Count+1 if OI_Data_15min[rows1[0]][6]-OI_Data_15min[rows1[0]][4] > 0 else Reduction_Count
			# 		Reduction_Count = Reduction_Count+1 if OI_Data_15min[rows2[0]][6]-OI_Data_15min[rows2[0]][4] > 0 else Reduction_Count

			# 		if Reduction_Count>=2:
			# 			if reversal_count==0:
			# 				print("Put Reduction in atleast 2 OI's" +  " time: "+ str(time_obj.time())+"  count:" + str(reversal_count+1))
			# 				reversal_count=reversal_count+1
			# 				a=nifty_data_after_format_change(time_obj,time_after_15_min)
			# 				if reversal_count==1:
			# 					trade_point=float(a[3])
			# 					reversal_count1_time=time_obj
			# 			elif reversal_count==1 and ( time_obj > (reversal_count1_time + timedelta(minutes=10)) ):
			# 				print("Put Reduction in atleast 2 OI's" +  " time: "+ str(time_obj.time())+"  count:" + str(reversal_count+1))
			# 				reversal_count=reversal_count+1
			# 				a=nifty_data_after_format_change(time_obj,time_after_15_min)
			# 				if reversal_count==2:
			# 					trade_point=min(trade_point,float(a[3]))
			# 					if reintiation:
			# 						retrade='PUT'
			# 						print(trade_point)
			# 					else:	
			# 						reversal_point_initiated='PUT'
			# 						reversal_point_initiated_time=time_obj
									
			# 		elif Reduction_Count==1 and reversal_count==1 and ( time_obj > (reversal_count1_time + timedelta(minutes=10)) and time_obj < (reversal_count1_time + timedelta(minutes=17))):
			# 			a=nifty_data_after_format_change(time_obj,time_after_15_min)
			# 			if float(a[3]) < trade_point:
			# 				print("Put Reduction in 1 OI's" +  " time: "+ str(time_obj.time())+"  count:" + str(reversal_count+1))
			# 				reversal_count=reversal_count+1
			# 				trade_point=float(a[3])
			# 				if reintiation:
			# 					retrade='PUT'
			...
			# 	if trade_side=='CALL':
			# 		rows=np.where(B==selected_strike_price[0])[0].tolist()
			# 		rows1=np.where(B==selected_strike_price[1])[0].tolist()
			# 		rows2=np.where(B==selected_strike_price[2])[0].tolist()
			# 		Reduction_Count=0
			# 		Reduction_Count = Reduction_Count+1 if OI_Data_15min[rows[0]][5]-OI_Data_15min[rows[0]][3] > 0 else Reduction_Count
			# 		Reduction_Count = Reduction_Count+1 if OI_Data_15min[rows1[0]][5]-OI_Data_15min[rows1[0]][3] > 0 else Reduction_Count
			# 		Reduction_Count = Reduction_Count+1 if OI_Data_15min[rows2[0]][5]-OI_Data_15min[rows2[0]][3] > 0 else Reduction_Count

			# 		if Reduction_Count>=2:
			# 			if reversal_count==0:
			# 				print("Call Reduction in atleast 2 OI's" +  " time: "+ str(time_obj.time())+"  count:" + str(reversal_count+1))
			# 				reversal_count=reversal_count+1
			# 				a=nifty_data_after_format_change(time_obj,time_after_15_min)
			# 				if reversal_count==1:
			# 					trade_point=float(a[2])
			...
			# 				a=nifty_data_after_format_change(time_obj,time_after_15_min)
			# 				if reversal_count==2:
			# 					trade_point=max(trade_point,float(a[2]))
			# 					if reintiation:
			# 						retrade='CALL'
			# 						print(trade_point)
			# 					else:	
			# 						reversal_point_initiated='CALL'
			# 						reversal_point_initiated_time=time_obj
									
			# 		elif Reduction_Count==1 and reversal_count==1 and ( time_obj > (reversal_count1_time + timedelta(minutes=10)) and time_obj < (reversal_count1_time + timedelta(minutes=17))):
			# 			a=nifty_data_after_format_change(time_obj,time_after_15_min)
			# 			if float(a[2]) > trade_point:
			# 				print("Call Reduction in 1 OI's" +  " time: "+ str(time_obj.time())+"  count:" + str(reversal_count+1))
			# 				reversal_count=reversal_count+1
			# 				trade_point=float(a[2])
			# 				if reintiation:
			# 					retrade='CALL'
			...
			"""Reversal intiated """
			# if reversal_point_initiated=='PUT' and  reintiation==False:
				df = pd.read_csv(path)
				if time_obj < (reversal_point_initiated_time + timedelta(minutes=60)):
			...
			# if reversal_point_initiated=='CALL' and reintiation==False:
			# 	df = pd.read_csv(path)
			# 	if time_obj < (reversal_point_initiated_time + timedelta(minutes=60)):
			# 		time_int2=time_obj - timedelta(minutes=5)
			# 		a=nifty_data_after_format_change(time_int2,time_obj)
			# 		print("Square Off Trade if Price Point goes above " + str(trade_point + 10))
			# 		if a==['']:
			# 			print("Ignore Once")
			# 		else:	
			...
			if(trade==False and reintiation==False):
