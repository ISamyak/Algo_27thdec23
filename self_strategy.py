...
folder_path = './Nirmal_Fincap/NIFTY_ADJ_OPT_2022/4/ex-date/'
...
def price_of_atm_strike_price(selected_itm_Sp,time):
	...
	filtered_data = df.loc[(df['Ticker']== selected_itm_Sp[2] ) & (df['Time'] == str(time))]
	extracted_data = filtered_data.to_dict('records')

	filtered_data  = df.loc[(df['Ticker']== selected_itm_Sp[3] ) & (df['Time'] == str(time) )]
	extracted_data2 = filtered_data.to_dict('records')

	price=[selected_itm_Sp[2],extracted_data[0]['Close'],selected_itm_Sp[3],extracted_data2[0]['Close']] 
...
with open(filepath, 'w', newline='') as file:
	...
	for i in files:
		...
		output_closing_time = output_date + " 144500"
		selected_itm_Sp = select_itm_strike_price(output_closing_time,[],0,331)
		...
		price=price_of_atm_strike_price(simple_filter_sp,time_obj.time())
		print(price)
		print("Sum of Price "+ str( float(price[1])+float(price[3])) )

		while time_obj.time() < datetime.strptime('15:29:59', '%H:%M:%S').time():
			time_obj += timedelta(minutes=m)
			n=n+1
			price=price_of_atm_strike_price(simple_filter_sp,time_obj.time())
			print(time_obj.time(),end="  ")
			print("Sum of Price "+ str( float(price[1])+float(price[3])) )
...
# @2pm COI - zero se start --- ATM,ITM,OTM Both side add on 10 lakh(depends on index) Oi which side , Opposite side me negotiable add (atleast 10-20%)	
#First best chnage ke 50 point piche wala sp sell kar do	-- only expiry  ,, 2no tarike se check kar skte hai - consider 50 as well and add 2 OTM,ITM